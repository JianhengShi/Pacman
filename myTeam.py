...
class Agent(CaptureAgent):
    ...
    #     dic3 = dict()
    #     for i in ls:
    #         if self.aStarSearch(gameState, gameState.getAgentState(self.index).getPosition(), [i], self.notGo2(gameState)) is not None and len(self.homeWay2(gameState, i))>0:
    #             dic3.update({i: len(self.aStarSearch(gameState, gameState.getAgentState(self.index).getPosition(), [i], self.notGo2(gameState)))})
    #         else:
    #             continue
    ...
    def CapOrHome(self, gameState):
        ...
        if gameState.getAgentState(self.index).numCarrying ==0:
            return self.eatDotsTest2(gameState)
        else:
            if len(way) == 0:
                return self.luckyWay(gameState)
            else:
    ...
    def homeWay(self, gameState):
        ...
        way = None
        a = 9999999
        if anop not in getMyLine(gameState,self.red) and anop not in getOppoLine(gameState,self.red):
            dp1=dp+[anop]
        else:
            dp1=dp
        for i in line:

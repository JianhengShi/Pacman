...
class Agent(CaptureAgent):
    ...
    def ifCatch(self, gameState):
        ...
        a = 0
        b = 0
        #Judge which of our agents is closer to the enemy
    ...
    def CapOrHome(self, gameState):
        ...
        # when finish eating 18 dots or time is up, we go home
    ...
    def notGo(self, gameState):
        ...
        for i in self.enemyGPosition(gameState):
            output=output+list(getAroundPositions(gameState, i)) # enemy ghost position and tiles adjacent are dangerous
            if i in getOppoLine(gameState, self.red):  
                for j in getAroundPositions(gameState, i):
                    if j in getMyLine(gameState, self.red) and gameState.getAgentState(self.index).scaredTimer == 0:
                        output.remove(j) # tiles in our side are safe (our agents become Ghost!)
            if self.getMazeDistance(i, gameState.getAgentState(self.index).getPosition())<=5:
                output=output+list(self.spl[2])
                output=output+list(self.spl[3]) # If enemy ghost nearby, going to a dead end or corrider leading to a dead end is dangerous
        ...
        # If we are Ghost and we are scared, position and positions nearby enemy Pacman are dangerous
        if gameState.getAgentState(self.index).scaredTimer > 0:
...
def getSpl(gameState, red):
    ...
    list1 = []
    map1 = {}
    target1 = set() # dead end position which only has one legal adjacent position but walls
        ...
        for y in range(1,gameState.data.layout.height):
            if not gameState.hasWall(x, y):
                ...
                if len(getAroundPositions(gameState, (x, y)))==1 :
                    ...
                    list1.append((x, y)) # not dead end
    target2 = set() # position whose all adjacent positions are dead ends
    a = len(list1)
    b = 0
    while a != b:
        a = len(list1)
    ...
    list2 = []
    map2 = {}
    target3 = set() # dead end position which only has one legal adjacent position but walls
        ...
        for y in range(1,gameState.data.layout.height):
            if not gameState.hasWall(x, y):
                ...
                if len(getAroundPositions(gameState, (x, y)))==1:
                    ...
                    list2.append((x, y))
    target4 = set() # position whose all adjacent positions are dead ends
    d = len(list2)
    e = 0
    while d != e:
        d = len(list2)
